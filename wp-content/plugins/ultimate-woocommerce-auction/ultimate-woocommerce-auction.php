...
if( ! function_exists( 'uwa_free_plugin_action_links' ) ) {
	...
	function uwa_free_plugin_action_links( $links ) {
        ...
        $links[] = '<a href="http://auctionplugin.net/" style="color: #389e38;font-weight: bold;" target="_blank">' . __( 'Get Pro', 'woo_ua' ) . '</a>';       
        $links[] = '<a href="' . admin_url( 'admin.php?page=uwa_general_setting' ) . '">' . __( 'Settings', 'woo_ua' ) . '</a>';
        $links[] = '<a href="https://docs.auctionplugin.net/" target="_blank">' . __( 'Documentation', 'woo_ua' ) . '</a>';
...
if ( in_array( 'woocommerce/woocommerce.php', $blog_plugins ) || isset( $site_plugins['woocommerce/woocommerce.php'] ) ) { 
	...
	if ( is_plugin_active( $pro_plugin ) ) {
		...
		function uwa_custom_message(){
		    ...
		    <div class="notice notice-error is-dismissible">
		        <p><?php _e( 'You are trying to activate Ultimate WooCommerce free version where as you already have its PRO version activated. Please deactivate Ultimate WooCommerce Pro version and then activate Free version.', 'woo_ua'); ?></p>
		    </div>
	...
	if ( ! class_exists( 'Ultimate_WooCommerce_Auction_Free' ) ) { 	 
		...
		if( !defined( 'WOO_UA_VERSION' ) ) {
			define( 'WOO_UA_VERSION', '2.0.10' ); // plugin version
		}
				...
				*/		
				...
				if ( function_exists( 'icl_object_id' ) && method_exists( $sitepress, 
					...
					add_action( 'ultimate_woocommerce_auction_place_bid', array( $this, 
						'uwa_syncronise_metadata_wpml' ), 1 );
					...
					add_action( 'woocommerce_process_product_meta', array( $this, 
						'uwa_syncronise_metadata_wpml' ), 85 );
			...
			*/
			...
			public function uwa_pro_add_plugins_notice() {
				...
				global $current_user;
					...
					if (isset($_GET['uwa_pro_add_plugin_notice_ignore']) && '0' == $_GET['uwa_pro_add_plugin_notice_ignore']) {
					...
					if (current_user_can('manage_options')) {
						...
						<div class="notice notice-info is-dismissible">
							...
							<div class="get_uwa_pro">
								<a href="https://auctionplugin.net?utm_source=woo plugin&utm_medium=admin notice&utm_campaign=learn-more-button" target="_blank"> <img src="<?php echo WOO_UA_ASSETS_URL;?>/images/UWCA_row.jpg" alt="" /> </a>
								
								<?php  
								echo '<p class="uwa_hide_free">
								' .sprintf(__('<a href="%s">Hide Notice</a>', 'woo_ua'),esc_attr(add_query_arg('uwa_pro_add_plugin_notice_ignore', '0'))) .'</p>';?> 
		...
		public function uwa_free_plugin_new_blog($blog_id, $user_id, $domain, $path, $site_id, $meta) {
			...
			*/		
			...
			public function uwa_syncronise_metadata_wpml( $data ){
				...
				$orginalid   = $sitepress->get_original_element_id( $product_id, 
					'post_product' );
	...
	if( ! function_exists( 'uwa_install_woocommerce_admin_notice' ) ) {
		...
		function uwa_install_woocommerce_admin_notice() { ?>			
			...
			<div class="updated" id="uwa-free-installer-notice" style="padding: 1em; position: relative;">
            	<h2><?php _e( 'Your Ultimate WooCommerce Auction is almost ready!', 'woo_ua' ); ?></h2>
	            ...
	            <?php else: ?>
	                ...
	                <p>
	                   ...
	                   <a class="install-now button" data-slug="woocommerce" 
	                   		href="<?php echo admin_url('plugin-install.php?tab=search&type=term&s=WooCommerce') ;?>" aria-label="Install WooCommerce 4.0.0 now" data-name="WooCommerce 4.0.0">Install Now</a>
	                </p>
	            <?php endif ?>
	        </div>
