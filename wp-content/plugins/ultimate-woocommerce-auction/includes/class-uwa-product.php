...
class WC_Product_Auction extends WC_Product {
	...
	*/
	public function __construct( $product ) {
		...
		$this->woo_ua_auction_item_condition_array = apply_filters( 'ultimate_woocommerce_auction_product_condition',array( 'new' => __('New', 'woo_ua'), 'used'=> __('Used', 'woo_ua') ));
	...
	*/
	function get_woo_ua_product_id(){
        ...
        elseif (function_exists('icl_object_id') && method_exists($sitepress, 
	...
	*/
	public function get_price_html( $price = '' ) {
		...
		if ($this->is_woo_ua_closed() && $this->is_woo_ua_started() ){
			...
			$price = '<span class="woo-ua-auction-price starting-bid" data-auction-id="'.$id.'" data-bid="'.$this->get_woo_ua_auction_current_bid().'" data-status="">'.__('<span class="uwa-starting auction">Starting bid</span>: ','woo_ua').wc_price($this->get_woo_ua_current_bid()).'</span>';
